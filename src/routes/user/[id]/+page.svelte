	...
	<div class="bg-gradient-to-br from-red-900 to-black rounded-lg shadow-lg p-6 text-white">
		...
		<div class="mt-8">
			<div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-4">
				...
				<div class="sm:hidden">
					<select
						class="w-full p-2 rounded-lg bg-red-800 text-white border-2 border-red-700"
			...
			{:else}
				<div class="space-y-4">
					{#each filteredVotes as vote}
						<div class="flex items-center space-x-4 p-4 bg-red-800/50 rounded-lg">
							...
							{:else}
								<div
									class="w-16 h-16 bg-gray-200 rounded flex items-center justify-center border-2 border-white"
								>
							...
							<div class="text-xl font-bold text-yellow-400">{vote.points} points</div>
						</div>
					{/each}
				</div>
			{/if}
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
					{#each data.publicGroups as group}
						<a href="/group/{group.group?.id}">
							<div
								class="p-4 bg-red-800/50 rounded-lg hover:bg-red-700/50 transition-colors"
							>
