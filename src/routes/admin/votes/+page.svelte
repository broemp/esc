	...
	<div class="card p-4">
					...
					{#each votes as vote}
						<tr>
							...
							<td class="px-4 md:px-6">
								...
								{:else}
									<span class="opacity-50">Deleted Act</span>
								{/if}
							</td>
							<td class="px-4 md:px-6">
								{#if vote.category}
									{vote.category.name}
								{:else}
									<span class="opacity-50">Deleted Category</span>
								{/if}
							</td>
		...
		<div class="flex justify-between items-center mt-4">
			<div class="text-sm opacity-60">
				...
				)} of {pagination.totalCount} votes
			</div>
			<div class="flex gap-2">
				...
				{#each Array(pagination.totalPages) as _, i}
					<button
						class="btn btn-sm {pagination.currentPage === i + 1 ? 'variant-filled-primary' : 'preset-tonal-primary'}"
