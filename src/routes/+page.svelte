...
<div class="w-full h-full grid grid-cols-1 content-center justify-items-center">
	<div class="w-full md:w-2/3 mx-auto flex flex-col items-center">
		{#if !$page.data.session}
			...
			<a href="/auth/signIn" class="btn btn-md preset-filled-primary-500 w-48">Join Now</a>
		{:else}
					...
					<div class="grid grid-cols-2 gap-4 mb-6">
						{#each data.publicGroups.slice(0, 4) as group}
							<a href="/group/{group.id}">
								<div class="card p-4 font-bold grid justify-items-center">
									<div>{group.name}</div>
									<div class="text-sm opacity-60">{group.memberCount} members</div>
								</div>
							</a>
						{/each}
					</div>
					<div class="flex gap-4 justify-center">
						<a href="/group/new" class="btn preset-filled-primary-500 w-48">Create New Group</a>
						<a href="/group/join" class="btn preset-filled-primary-500 w-48">View All Groups</a>
					</div>
				</div>
			{/if}

			<div class="card p-4 grid grid-cols-[auto_1fr_auto] gap-4 items-center">
				<button type="button" class="btn-icon preset-filled" onclick={carouselLeft}>
				...
				>
					{#each acts as act}
						<img
							class="snap-center object-contain w-[1024px] max-h-32 rounded cursor-pointer"
				...
				<button type="button" class="btn-icon preset-filled" onclick={carouselRight}>
			...
			{#if data.groups.length > 0}
				<p class="text-center pt-2 text-2xl underline decoration-2 underline-offset-2">
					Your Groups
				</p>
				<div class="m-4">
					<div class="grid grid-cols-2 gap-4">
						{#each data.groups as group, i}
							<a
								...
								class={i === data.groups.length - 1 && data.groups.length % 2 === 1
									? 'col-span-2'
									: ''}
							>
								<div class="card p-4 font-bold grid justify-items-center">
				...
				<div class="flex p-4 gap-4">
					<a href="/group/new" class="btn preset-filled-primary-500 w-1/2">Create new Group</a>
					<a href="/group/join" class="btn preset-filled-primary-500 w-1/2">Join Group</a>
				</div>
			{/if}
			...
			{:else}
				<div class="text-center opacity-60 m-4">No public groups available</div>
			{/if}
