...
export const groups = pgTable("group", {
  ...
  admin: text("adminID").notNull().references(() => users.id, { onDelete: "cascade" }),
...
export const userInGroups = pgTable("user_group", {
...
export const categoriesInGroup = pgTable("category_group", {
  categoryId: uuid("categories_id").notNull().references(() => categories.id),
  groupId: uuid("group_id").notNull().references(() => groups.id),
},
  (table) => {
    return {
...
export const acts = pgTable("act", {
  id: uuid("id").notNull().primaryKey()
    .defaultRandom(),
  countryID: uuid("country_id").notNull().references(
...
export const votes = pgTable("vote", {
  ...
  actID: uuid("actID").notNull().references(
    () => acts.id, { onDelete: "cascade" }),
  categories: uuid("categoriesID").notNull().references(
